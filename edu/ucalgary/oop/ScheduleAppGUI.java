...
public class ScheduleAppGUI extends JFrame {
    ...
    public ScheduleAppGUI() {
        ...
        calculateButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                Animal animalInfo = new Animal();   //initial 8 coyotes
                db.createConnection();
                // Task Task = new Task();

                try {
                    Coyote.feedingTimeCoyote();
                } catch (InsufficientFeedingTimeException error) {
                    JOptionPane.showMessageDialog(null, "Could not schedule feeding for all coyotes ", "Error", JOptionPane.ERROR_MESSAGE);              
                  }  
                 try {
                    Fox.feedingTimeFox();
                 } catch (InsufficientFeedingTimeException error) {
                    JOptionPane.showMessageDialog(null, "Could not schedule feeding for all Fox please update the database ", "Error", JOptionPane.ERROR_MESSAGE);              
                 } 
                try {
                  Raccoon.feedingTimeRaccoon();
                } catch (InsufficientFeedingTimeException error) {
                    JOptionPane.showMessageDialog(null, "Could not schedule feeding for all Raccoon please update the database", "Error", JOptionPane.ERROR_MESSAGE);              
                } 
                try {
                    Beaver.feedingTimeBeaver();
                } catch (InsufficientFeedingTimeException error) {
                    JOptionPane.showMessageDialog(null, "Could not schedule feeding for all Beaver please update the database ", "Error", JOptionPane.ERROR_MESSAGE);              
                } 
                try {
                  Porcupine.feedingTimePorcupine();
                } catch (InsufficientFeedingTimeException error) {
                    JOptionPane.showMessageDialog(null, "Could not schedule feeding for all Porcupines please update the database", "Error", JOptionPane.ERROR_MESSAGE);              
                }                     
                Coyote.addCleaningCoyoteTimesToHashmap(db,animalInfo);
                Beaver.addCleaningBeaverTimesToHashmap(db,animalInfo);
                Fox.addCleaningFoxTimesToHashmap(db,animalInfo);
                Porcupine.addCleaningPorcupineTimesToHashmap(db,animalInfo);
                Raccoon.addCleaningRaccoonTimesToHashmap(db,animalInfo);
    ...
    public static void main(String[] args) {
        /*These are the commands to run this code: (The lib folder should be right outside the edu.ucalgary.oop package)
         * Run these commands from the directory right outside the edu.ucalgary.oop package
         * Comment out AllTesting class before running these commands
         * javac -cp .;lib/mysql-connector-java-7.0.23.jar edu/ucalgary/oop/*.java
         * java -cp .;lib/mysql-connector-java-8.0.23.jar edu.ucalgary.oop.ScheduleAppGUI
         */
        new ScheduleAppGUI();
